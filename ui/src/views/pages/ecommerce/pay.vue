...
export default {
    ...
    methods: {
        placeOrder() {
            ...
            axios.post('oerderadd/', formData).then(() => {
                //send stk push
                axios.post('stkpush/', {
        ...
        <div class="row">
                        ...
                        <div class="p-3 bg-light mb-4">
                            <h5 class="font-size-16 mb-0 text-uppercase">
                                ...
                                <span class="float-end ms-2 badge badge-pill bg-warning"
                                    v-if="showdetails">#{{ orderNo }}</span>
                                <a class="link mb-2  d-inline float-end mr-4" href="#" @click="showdetails = true"
                                    v-if="!showdetails">Show details<i class="fa fa-plus"></i></a>
                                <a class="link mb-2 d-inline float-end mr-4" href="#" @click="showdetails = false"
                                    v-if="showdetails">Hide details<i class="fa fa-minus"></i></a>
                            </h5>
                        </div>
                            ...
                            <table class="table table-centered mb-0 table-nowrap">
                                ...
                                <tbody>
                                    <tr v-for="item in cart.items" :key="item.id" v-show="showdetails">
                                        <th scope="row">
                                            <img :src="item.product.images[0].image" alt="product-img" title="product-img"
                                                class="avatar-md" />
                                        </th>
                                        <td>
                                            <h5 class="font-size-14 text-truncate">
                                                <router-link
                                                    :to="{ name: 'Product Detail', params: { product: item.product, category: item.product.maincategory } }"
                                                    class="text-dark text-truncate">{{ item.product.title }}</router-link>
                                            </h5>
                                            <p class="text-muted mb-0">{{ item.item_subtotal }} x {{ item.quantity }}</p>
                                        </td>
                                        <td>{{ new Intl.NumberFormat().format(item.item_total * item.quantity) }}</td>
                                    </tr>
                                    <tr v-show="showdetails">
                                        <td colspan="2">
                                    ...
                                    <tr class="overflow-hidden">
                                        <td class="d-inline">
                                            <h3 class="font-size-15 text-uppercase text-muted">Total to pay
            ...
            <div class="col-sm-12">
                <div class="custom-accordion">
                    <h5 class="font-size-16 mb-1 text-center p-2 bg-light text-muted" v-if="prevPayments">YOU WILL PAY WITH
                    </h5>
                    <div class="card" v-if="prevPayments">
                        <div class="card-body">
                            ...
                            <v-divider></v-divider>
                            <div class="row">
                                <a class="link text-uppercase font-size-16 font-weight-bold text-center">use a different
                                    payment
                                    method</a>
                            </div>
                        </div>
                    ...
                    <div class="card">
                        <div class="card-body m-auto">
                            <button class="btn btn-primary font-size-16 p-2" v-b-modal.modal-confirmdetails>PAY NOW KES
                                {{ new Intl.NumberFormat().format(cart.total) }}</button>
                        </div>
                    </div>
                </div>

                <div class="row my-4">
                    <p class="font-size-14">By tapping "PAY NOW" I aggree to Bengomall`s <a class="link" href="#">Payment
                            Terms & Conditions</a>,<a class="link" href="#"> General
                            Terms & Conditions</a>, and <a class="link" href="#">Privacy and Cockies
                            Notice</a>
                    </p>
                    <v-divider height="5"></v-divider>
                    <p class="font-size-14"><b>NOTE:</b>BengoPay will never ask for your Password,PIN,CVV or full card
                        details over the phone or via email.</p>
                    <div class="row">
                        <p class="text-center font-size-16">Need help?<a href="#" class="link">Contact us</a></p>
                    </div>
                    <div class="col  text-center">
                        <v-divider height="8"></v-divider>
                        <router-link to="/ecommerce/products" class="btn btn-link text-muted text-uppercase font-size-18">
