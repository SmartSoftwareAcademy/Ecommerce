    ...
    <div class="container overflow-auto">
        <v-card class="receipt-card" ref="receipt" id="receipt">
            ...
            <v-card-text class="receipt-text">
                ...
                <p>------------------------------------</p>
                <v-data-table :headers="headers" :items="items" item-key="id" hide-default-footer class="hide-per-page">
                    <template v-slot:[`item.title`]="{ item }" class="text-wrap">{{ item.product.title }}</template>
                    <template v-slot:[`item.price`]="{ item }">{{ item.price }}</template>
                    <template v-slot:[`item.quantity`]="{ item }">{{ item.quantity }}</template>
                </v-data-table>
                <p>------------------------------------</p>
                <v-row>
                    <v-col class="font-weight-bold">Subtotal: {{ formatCurrency(subtotal * total) }}</v-col>
                </v-row>
                <v-row>
